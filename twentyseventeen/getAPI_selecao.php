<?php
$executandoDireto = !defined('CURSOS_SELECAO_INTERNO');
...
if (!isset($mneumonico)) {
    $entradaMnemonico = $_GET['mnemonico'] ?? $_GET['codigo'] ?? null;
    if (is_string($entradaMnemonico) && $entradaMnemonico !== '') {
        $mneumonico = $entradaMnemonico;
    }
}
...
if (empty($mneumonico)) {
    if ($executandoDireto) {
        ...
        echo json_encode(['error' => 'Parâmetro mnemonico não informado.']);
        exit;
    }
    return;
}
